const Select = ({
  ...
  return (
    <label style={style} className={`block ${className}`}>
      <span className={withLabel ? 'block text-gray-300 text-sm font-medium mb-1' : 'sr-only'}>{label}</span>
      <select
        value={value}
        onChange={onChange}
        className={
          `block w-full bg-gray-900 rounded-md ${border} shadow-sm focus:border-green-700 focus:ring focus:ring-green-800 focus:ring-opacity-50`
        }
        {...rest}
      >
        {
